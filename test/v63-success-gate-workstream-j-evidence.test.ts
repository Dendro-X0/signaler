...
describe("v6.3 success gate workstream-j overhead evidence check", () => {
  it("marks check ok when workstream-j overhead evidence is pass", async () => {
    const root = await mkdtemp(join(tmpdir(), "signaler-v63-gate-j-overhead-ok-"));
    try {
      ...
      const check = checkStatus(report.checks, "workstream-j-overhead-evidence");
  ...
  it("marks check warn when workstream-j overhead evidence is missing", async () => {
    const root = await mkdtemp(join(tmpdir(), "signaler-v63-gate-j-overhead-warn-"));
    try {
      await scaffoldGateRoot(root);
      const report = await evaluateSuccessGate({
        rootDir: root,
