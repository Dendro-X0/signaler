...
describe("v6.3 success gate repo-validation evidence check", () => {
  it("marks check ok when repo validation evidence has >=2 qualified repos", async () => {
    const root = await mkdtemp(join(tmpdir(), "signaler-v63-gate-repo-validation-ok-"));
    try {
      await scaffoldGateRoot(root);
      await writeText(
        resolve(root, "release/v3/repo-validation-evidence.json"),
        JSON.stringify(
          {
            schemaVersion: 1,
      ...
      const check = checkStatus(report.checks, "repo-validation-evidence");
      expect(check.status).toBe("ok");
      expect(check.details).toContain("complete");
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  });

  it("marks check warn when repo validation evidence is missing", async () => {
    const root = await mkdtemp(join(tmpdir(), "signaler-v63-gate-repo-validation-warn-"));
    try {
      await scaffoldGateRoot(root);
      const report = await evaluateSuccessGate({
        rootDir: root,
