# Signaler Documentation

This folder contains practical docs for the current `@signaler/cli` codebase.

## Start Here

- [Current Shortcomings (Reality Check)](CURRENT-SHORTCOMINGS.md)
- [Getting Started](getting-started.md)
- [Accuracy Spec](accuracy-spec.md)
- [V3 Contract](v3-contract.md)
- [Migration V3](migration-v3.md)
- [CLI and CI](cli-and-ci.md)
- [Configuration and Routes](configuration-and-routes.md)
- [API Reference](api-reference.md)
- [Features](FEATURES.md)
- [Troubleshooting](troubleshooting.md)

## Examples

- [Basic Usage](examples/basic-usage.md)
- [CLI Usage](examples/cli-usage.md)
- [Configuration Examples](examples/configuration-examples.md)
- [Framework Integration](examples/framework-integration.md)
- [Advanced Usage](examples/advanced-usage.md)

## Release and Migration Notes

- [Migration](MIGRATION.md)
- [Release Notes v2.0](RELEASE-NOTES-v2.0.md)
- [Release Notes v2.1](RELEASE-NOTES-v2.1.md)
- [Release Notes v2.4](RELEASE-NOTES-v2.4.md)
- [Release Notes v2.5.1](RELEASE-NOTES-v2.5.1.md)
- [Release Notes v2.5.2](RELEASE-NOTES-v2.5.2.md)
- [Release Notes v2.6.4](RELEASE-NOTES-v2.6.4.md)

## Quick Install

```bash
npx jsr add @signaler/cli
```

## Quick Run

```bash
signaler init
signaler run --contract v3 --mode throughput
signaler review
```

## Support

- Issues: <https://github.com/Dendro-X0/signaler/issues>
- Main README: ../README.md
