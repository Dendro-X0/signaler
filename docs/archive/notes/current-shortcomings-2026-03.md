...
## 8. Parallel-Reduction Trap in Large Fidelity Suites

Severity: High

- For large route/device suites, forcing very low parallelism (especially `parallel=1`) in fidelity/devtools mode can:
  - dramatically increase runtime
  - still produce lower performance scores on later combos
- This creates a false optimization loop where users spend more time but do not gain better parity.

Impact:
- Wasted audit time.
- Lower confidence in parity validation.
- Misleading assumption that "lower parallel always means more accurate."

Current guidance:
- Use full-suite `throughput` for detection and trend.
- Use focused `fidelity` reruns for parity checks (worst routes only).
...
## V4 Phase 0 Gate (Contract Freeze)

Use this checklist before Phase 1 implementation starts.

- [x] Canonical commands are frozen in docs: `discover -> run -> report`.
- [x] Alias policy is documented: `init -> discover`, `review -> report`.
- [x] Canonical artifacts are frozen in docs:
  - `.signaler/run.json`
  - `.signaler/results.json`
  - `.signaler/suggestions.json`
  - `.signaler/report.md`
- [x] Migration guidance exists for both current stable flow and V4 target flow.
- [x] `ROADMAP.md`, `README.md`, and `/docs` references are aligned to V4 reset direction.
