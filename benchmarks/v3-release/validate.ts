...
type V3ReleaseGateReport = z.infer<typeof reportSchema>;

export type V3ReleaseGateValidationResult =
  | { readonly ok: true; readonly value: V3ReleaseGateReport }
...
function validateSummary(report: V3ReleaseGateReport): readonly string[] {
...
export async function validateV3ReleaseGateReportFile(pathToReport: string): Promise<V3ReleaseGateValidationResult> {
  try {
...
async function main(): Promise<void> {
  const reportPath = process.argv[2] ?? "benchmarks/out/v3-release-gate.json";
  const schemaPath = resolve("benchmarks/v3-release/report.schema.json");
  ...
  console.log(`V3 release gate report is valid: ${resolve(reportPath)}`);
}
